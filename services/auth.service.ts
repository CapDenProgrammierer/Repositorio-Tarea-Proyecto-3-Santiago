...
export class AuthService {
  ...
  public hasRole(role: string): boolean {
    ...
    // 1) authorities: [{ authority: 'ROLE_SUPER_ADMIN' }]
    const authorities = (this.user as any).authorities;
    if (Array.isArray(authorities)) {
      for (const a of authorities) {
        const authVal = normalize(a?.authority ?? a); // acepta string u objeto
        if (authVal === wanted || authVal === `ROLE_${wanted}` || `ROLE_${authVal}` === wanted) return true;
      }
    }

    // 2) role object: { name: 'SUPER_ADMIN' }
    const roleObjName = normalize((this.user as any).role?.name);
    if (roleObjName) {
      if (roleObjName === wanted || `ROLE_${roleObjName}` === wanted || roleObjName === wanted.replace(/^ROLE_/, '')) return true;
    }
  ...
  public isSuperAdmin(): boolean {
    // acepta 'SUPER_ADMIN' y 'ROLE_SUPER_ADMIN' y también compatibilidad con 'ADMIN' si lo deseas
    return this.hasRole('SUPER_ADMIN') || this.hasRole('ROLE_SUPER_ADMIN') || this.hasRole((IRoleType as any)?.superAdmin ?? 'SUPER_ADMIN');
  }
